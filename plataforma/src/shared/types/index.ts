...
// Estado da assinatura. A criação de usuário só acontece após pagamento aprovado
// na landing page; o app não tem mais self-signup.
export type SubscriptionStatus =
  | "pending_approval" // pagamento confirmado, aguardando aprovação do admin
  | "active" // aprovado pelo admin, acesso liberado
  | "pending_payment" // pagamento pendente (boleto, etc.)
  | "past_due" // pagamento recorrente falhou
...
export interface DoctorProfile {
  ...
  instagram?: string;
  avatarUrl?: string;
  // false na criação (via pagamento); vira true ao concluir o onboarding.
  // Controla o redirecionamento obrigatório para o onboarding no primeiro acesso.
...
export interface AnalyticsSummary {
  ...
  // Instagram Business Login API — zero / false quando não conectado
  igConnected: boolean;
  ...
  // Per-post insights (instagram_business_manage_insights)
  mediaInsights: IGMediaInsight[];
}
