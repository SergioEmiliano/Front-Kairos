...
import { MetricCard } from "@/app/(plataform)/dashboard/analytics/components/MetricCard";
import { AIAnalystPanel } from "@/app/(plataform)/dashboard/analytics/components/AIAnalystPanel";
import { FloatingAnalystButton } from "@/app/(plataform)/dashboard/analytics/components/FloatingAnalystButton";
import { analyticsService } from "@/services/analytics.service";
import { useUIStore } from "@/store/ui.store";
import { AnalyticsSummary, IGMediaInsight } from "@/shared/types";
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Alcance", value: "8,4K", sub: null }, // média semanal: conectar após ter data
...
function RevenueFunnel({ data }: { data: AnalyticsSummary }) {
  const igOk = data.igConnected;
  const steps = [
    {
      emoji: "📝",
      value: "28", // TODO: conectar com content API
      name: "Conteúdos Produzidos",
      detail: "Calendário Kairós\nConteúdo publicado no período",
      rate: null, // reach não é taxa de conversão de conteúdo
      highlight: false,
    },
    {
      emoji: "👁️",
      value: igOk ? fmtK(data.totalReach) : "—",
      name: "alcance total",
      detail: igOk ? "Fonte: Instagram API\nSérie temporal diária" : "Instagram não conectado",
      rate: igOk ? pct(data.profileViews, data.totalReach) : null,
      highlight: false,
    },
    {
      emoji: "👤",
      value: igOk ? data.profileViews.toLocaleString("pt-BR") : "—",
      name: "visitas ao perfil",
      detail: igOk ? "Fonte: Instagram API\nTotal do período" : "Instagram não conectado",
      rate: pct(data.totalDms, data.profileViews),
      highlight: false,
    },
    {
      emoji: "💬",
      ...
      rate: pct(data.totalAppointments, data.totalDms),
      highlight: true,
      gold: true,
    },
    {
    ...
    {
      emoji: "💰",
      value: fmtBRL(data.monthlyRevenueCurrent),
      name: "faturamento",
      detail: `${Math.round(data.revenuePercent)}% da meta mensal\nFonte: check-in`,
      rate: null,
      highlight: true,
      gold: true,
    },
    ...
    <div className="chart-wrap p-0 overflow-hidden">
      ...
      >
        <div>
          ...
          <SectionTitle>
            Entenda como seu conteúdo se{" "}
            <span className="italic-gold">transforma em receita</span>
          </SectionTitle>
        </div>
...
function FormatPerformance({ data }: { data: AnalyticsSummary }) {
  const posts = data.mediaInsights ?? [];
  const igOk = data.igConnected && posts.length > 0;

  const grouped: Record<string, IGMediaInsight[]> = {
    Reel: [], Carrossel: [], Post: [], Story: [],
  };
  for (const p of posts) {
    if (p.mediaType === "REELS" || p.mediaType === "VIDEO") grouped.Reel.push(p);
    else if (p.mediaType === "CAROUSEL_ALBUM") grouped.Carrossel.push(p);
    else if (p.mediaType === "IMAGE") grouped.Post.push(p);
  }

  const avgReach = (items: IGMediaInsight[]) =>
    items.length > 0 ? Math.round(items.reduce((s, p) => s + p.reach, 0) / items.length) : 0;

  const avgValues = {
    Reel: avgReach(grouped.Reel),
    Carrossel: avgReach(grouped.Carrossel),
    Post: avgReach(grouped.Post),
    Story: 0,
  };
  const maxAvg = Math.max(...Object.values(avgValues), 1);

  const formats = (["Reel", "Carrossel", "Post", "Story"] as const).map((label) => ({
    label,
    count: grouped[label]?.length ?? 0,
    value: avgValues[label],
    pct: igOk ? Math.round((avgValues[label] / maxAvg) * 100) : 0,
    delta: "—",
    ...
    <div className="chart-wrap flex flex-col gap-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <SectionTitle>Média por publicação · {new Date().toLocaleString("pt-BR", { month: "long", year: "numeric" })}</SectionTitle>
        </div>
        <span className="mono-label" style={{ fontSize: 9 }}>
          {igOk ? `${posts.length} publicações` : "Sem dados"}
        </span>
      </div>
      ...
      >
        <div className="text-[11.5px] font-medium text-kairos-charcoal">
          {igOk ? "✓ Media API conectada" : "ℹ️ Dados por formato"}
        </div>
        <div className="text-[11.5px] text-kairos-stone mt-1 leading-relaxed">
          {igOk
            ? `Alcance médio calculado a partir de ${posts.length} publicações via instagram_business_manage_insights.`
            : "Alcance por formato exige acesso à Media API (instagram_business_manage_insights). Conecte o Instagram para ver os dados."}
        </div>
      </div>
    </div>
  );
...
function PillarPerformance() {
  // Alcance por pilar exige per-post insights + tag de pilar por post — não disponível
  // no app Kairos 2-IG. Depende da Media API e da categorização de conteúdo.
  const pillars = [
    { name: "Autoridade", pct: 0, reach: "—", fill: 0, hi: false },
    { name: "Educação",   pct: 0, reach: "—", fill: 0, hi: false },
    { name: "Conexão",    pct: 0, reach: "—", fill: 0, hi: false, steel: true },
    { name: "Conversão",  pct: 0, reach: "—", fill: 0, hi: false, faded: true },
  ];
  ...
  return (
    <div className="chart-wrap flex flex-col gap-4">
      ...
      <div className="text-[11px] text-kairos-stone leading-relaxed">
        Performance por pilar exige <strong className="text-kairos-charcoal">per-post insights</strong> e categorização do conteúdo por pilar. Disponível via app Kairos 2 + Media API.
      </div>
    </div>
  );
}

function ReelDiagnostic({ data }: { data: AnalyticsSummary }) {
  const reels = (data.mediaInsights ?? [])
    .filter((p) => p.mediaType === "REELS" || p.mediaType === "VIDEO")
    .sort((a, b) => b.reach - a.reach)
    .slice(0, 10)
    .map((p) => ({
      title: (p.caption || "Sem legenda").slice(0, 55) + ((p.caption?.length ?? 0) > 55 ? "…" : ""),
      meta: p.timestamp,
      reach: fmtK(p.reach),
      saves: p.saved,
      savesGold: p.saved > 30,
      skip: "—",
      skipV: "amber" as "green" | "amber" | "red",
      watch: 0,
      watchSec: "—",
      status: p.reach >= 1000 ? "Bom" : "Médio",
      statusV: (p.reach >= 1000 ? "green" : "amber") as "green" | "amber" | "red",
    }));
    ...
    <div className="chart-wrap">
      ...
      <table className="w-full" style={{ borderCollapse: "collapse" }}>
        ...
        <tbody>
          {reels.length === 0 && (
            <tr>
              <td colSpan={6} className="py-8 text-center text-[12px] text-kairos-stone">
                {data.igConnected
                  ? "Nenhum Reel publicado no período"
                  : "Conecte o Instagram para ver o diagnóstico por Reel"}
              </td>
            </tr>
          )}
          {reels.map((r, ri) => (
...
function TopSaves({ data }: { data: AnalyticsSummary }) {
  const formatEmoji: Record<string, string> = {
    REELS: "🎬", VIDEO: "🎬", CAROUSEL_ALBUM: "📋", IMAGE: "📸",
  };
  const posts = (data.mediaInsights ?? [])
    .filter((p) => p.saved > 0)
    ...
    .map((p, i) => ({
      rank: `0${i + 1}`,
      emoji: formatEmoji[p.mediaType] ?? "📸",
      title: (p.caption || "Sem legenda").slice(0, 60) + ((p.caption?.length ?? 0) > 60 ? "…" : ""),
      meta: `${p.mediaType} · ${p.timestamp} · ${p.reach.toLocaleString("pt-BR")} alcance`,
      saves: p.saved,
    }));
  ...
  return (
    <div className="chart-wrap">
      <div className="flex items-start justify-between mb-3">
        <div>
          <SectionKicker>Top 5 Conteúdos</SectionKicker>
          <SectionTitle>Os posts que mais geraram valor para o seu público</SectionTitle>
        </div>
        <span className="mono-label" style={{ fontSize: 9 }}>Save = lead qualificada</span>
      </div>

      {posts.length === 0 && (
        <div className="py-8 text-center text-[12px] text-kairos-stone">
          {data.igConnected
            ? "Nenhum post com saves no período"
            : "Conecte o Instagram para ver o ranking de saves"}
        </div>
      )}
...
function ContentInsightPanel({ data }: { data: AnalyticsSummary }) {
  const igOk = data.igConnected;
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <div className="text-[12px] text-kairos-stone leading-relaxed">
          Saves indicam que a audiência quer revisitar o conteúdo sinal claro de intenção de agendamento futuro.
        </div>
        <div className="hairline" />
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <span className="mono-label" style={{ fontSize: 9 }}>Total saves / mês</span>
            <span className="text-[22px] font-medium italic-gold" style={{ letterSpacing: "-0.025em" }}>
              {igOk ? data.saves.toLocaleString("pt-BR") : "—"}
            </span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="mono-label" style={{ fontSize: 9 }}>Avg por Reel</span>
            <span className="text-[22px] font-medium text-kairos-charcoal" style={{ letterSpacing: "-0.025em" }}>
              —
            </span>
          </div>
        </div>
      ...
      <div className="chart-wrap flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-center gap-5">
          {[
            { label: "Shares", value: igOk ? data.shares.toLocaleString("pt-BR") : "—", muted: false },
            { label: "Saves", value: igOk ? data.saves.toLocaleString("pt-BR") : "—", muted: false },
            { label: "Likes", value: igOk ? data.likes.toLocaleString("pt-BR") : "—", muted: true },
...
function BeforeAfterChart({ data }: { data: AnalyticsSummary }) {
  const currentReach = data.igConnected ? fmtK(data.totalReach) : "—";
  return (
    <div className="chart-wrap">
      <div className="flex items-start justify-between mb-4">
        <div>
      ...
      >
        ...
        <text x="408" y="47" fontFamily="Inter Tight,system-ui" fontSize="13"
          fill="oklch(0.560 0.092 68)" textAnchor="middle" fontWeight="500" fontStyle="italic">{currentReach}</text>
      </svg>
    </div>
  );
}
...
function BenchmarkPanel({ data }: { data: AnalyticsSummary }) {
  const convVal = data.totalDms > 0
    ? Math.round((data.totalAppointments / data.totalDms) * 100) + "%"
    : "—";
  const convPct = data.totalDms > 0
    ? Math.min(Math.round((data.totalAppointments / data.totalDms) * 200), 100)
    : 0;

  const perfDmRatio = data.igConnected && data.profileViews > 0
    ? (data.totalDms / data.profileViews) * 100
    : null;
  const perfDmVal = perfDmRatio !== null ? perfDmRatio.toFixed(1) + "%" : "—";
  const perfDmPct = perfDmRatio !== null ? Math.min(Math.round((perfDmRatio / 4.1) * 65), 100) : 0;
  const perfDmWarn = perfDmRatio !== null ? perfDmRatio < 4.1 : false;

  const rows = [
    {
      label: "Alcance mensal",
      youPct: data.igConnected ? Math.min(Math.round((data.totalReach / 23100) * 58), 100) : 0,
      avgPct: 58,
      youVal: data.igConnected ? fmtK(data.totalReach) : "—",
      avgVal: "23,1K",
      warn: false,
    },
    {
      label: "Leads / mês",
      youPct: Math.min(Math.round((data.totalDms / 39) * 60), 100),
      avgPct: 60,
      youVal: String(data.totalDms),
      avgVal: "39",
      warn: false,
    },
    {
      label: "Conv. DM→Agenda",
      youPct: convPct,
      avgPct: 52,
      youVal: convVal,
      avgVal: "22%",
    ...
    { label: "Saves / post",    youPct: 0,          avgPct: 47, youVal: "—",       avgVal: "57",    warn: false },
    { label: "Watch time Reels",youPct: 0,          avgPct: 50, youVal: "—",       avgVal: "11,8s", warn: false },
    {
      label: "Perfil→DM",
      youPct: perfDmPct,
      avgPct: 65,
  ...
  return (
    <div className="chart-wrap flex flex-col gap-4">
      ...
      >
        <div className="text-[11.5px] font-medium text-kairos-charcoal">
          {perfDmWarn ? "⚠️ Métrica abaixo da média" : "ℹ️ Benchmark de referência"}
        </div>
        <div className="text-[11px] text-kairos-stone mt-1 leading-relaxed">
          {perfDmWarn
            ? `Perfil→DM em ${perfDmVal} vs. média de 4,1%. Melhorar o CTA na bio e nos Stories é a maior alavanca disponível.`
            : "Saves/post e Watch time exigem per-post insights (app Kairos 2). Dados de alcance, leads e conversão já integrados via Instagram API e check-in."}
        </div>
      </div>
    </div>
  );
            ...
            <div className="flex items-center gap-2.5 mt-1 shrink-0">
              ...
              {data.igConnected ? (
                ...
                >
                  ...
                  {data.instagramUsername || "Instagram"} conectado
                </div>
              ) : (
                <div
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-medium"
                ...
                >
                  <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: "var(--mute)" }} />
            ...
            <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="grid grid-cols-4 gap-4">
                <MetricCard
                  ...
                  value={data.igConnected ? fmtK(data.totalReach) : "—"}
                  subtitle={data.igConnected ? "INSTAGRAM · API META" : "CONECTE O INSTAGRAM"}
                  icon={TrendingUp}
                  progress={data.igConnected ? Math.min(Math.round(data.totalReach / 500), 100) : 0}
                  index={0}
                />
                <MetricCard
                  ...
                  subtitle="CHECK-IN"
                  icon={Users}
                  progress={Math.min(data.totalDms, 100)}
                  index={1}
                  italic
                />
                <MetricCard
                  title="Conversão agenda"
                  value={data.totalDms > 0 ? Math.round(data.totalAppointments / data.totalDms * 100) + "%" : "—"}
                  subtitle="CHECK-IN"
                  icon={Target}
                  progress={data.totalDms > 0 ? Math.min(Math.round(data.totalAppointments / data.totalDms * 200), 100) : 0}
                  index={2}
                />
                <MetricCard
                  title="Faturamento"
                  value={fmtBRL(data.monthlyRevenueCurrent)}
                  subtitle={`${Math.round(data.revenuePercent)}% DA META · CHECK-IN`}
                  icon={BarChart3}
                  progress={Math.min(Math.round(data.revenuePercent), 100)}
                  index={3}
                  italic
                />
              </div>
          ...
          {activeTab === "performance" && (
            <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormatPerformance data={data} />
                <PillarPerformance />
              </div>

              {/* Reel diagnostic */}
              <ReelDiagnostic data={data} />

              {/* Top saves + insight panel */}
              <div className="grid grid-cols-2 gap-4">
                <TopSaves data={data} />
                <ContentInsightPanel data={data} />
              </div>
            </div>
          )}
