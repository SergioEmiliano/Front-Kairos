...
type QRadio    = { type: "radio";    label: string; options: string[]; dependsOn?: string };
...
const BLOCKS: Block[] = [
  ...
  },
  {
    ...
    questions: [
      { type: "chips", label: "Quais procedimentos representam sua clínica?", options: ["Harmonização facial", "Bioestimulador", "Preenchimento labial", "Skinbooster", "Botox / Toxina", "Laser / Fotona", "Peelings e revitalização", "Fios e lifting", "Procedimento cirúrgico", "Outros procedimentos"] },
      { type: "radio", label: "Qual procedimento você mais deseja vender?", options: ["Harmonização facial", "Bioestimulador", "Preenchimento labial", "Skinbooster", "Botox / Toxina", "Laser / Fotona", "Peelings e revitalização", "Fios e lifting", "Procedimento cirúrgico", "Outros procedimentos"], dependsOn: "Quais procedimentos representam sua clínica?" },
  ...
  },
  {
    ...
    questions: [
      { type: "chips", label: "Como sua clínica deve ser percebida?", options: ["Sofisticada", "Premium", "Acolhedora", "Técnica", "Moderna", "Natural"], max: 3 },
      { type: "radio", label: "Qual estilo de comunicação combina com sua clínica?", options: ["Elegante", "Humanizada", "Técnica", "Moderna", "Premium", "Próxima"] },
  ...
  },
  {
    ...
    questions: [
      { type: "radio", label: "Quem é seu público principal?", options: ["Mulheres 25–35", "Mulheres 35–50", "Público premium", "Primeira harmonização", "Pacientes recorrentes"] },
  ...
  },
  {
    ...
    questions: [
      { type: "radio", label: "Qual é seu principal objetivo hoje?", options: ["Crescimento rápido", "Agenda lotada", "Autoridade no nicho", "Posicionamento premium", "Crescimento sustentável"] },
  ...
  return (
    <div className="flex flex-col gap-6">
      {block.questions.map((q) => {
        ...
        if (q.type === "radio") {
          const opts = q.dependsOn
            ? ((answers[q.dependsOn] as string[]) ?? []).filter(Boolean)
            : q.options;
          const effectiveOpts = opts.length > 0 ? opts : q.options;
          return (
